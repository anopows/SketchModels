...
class LSTM(SketchModel):
    def __init__(self, init_flags=True, my_flags=None):
        super(LSTM, self).__init__()

    def _init_flags(self):
        super(LSTM, self)._init_flags()
        self.FLAGS['prefix'].value = 'LSTM'
    ...
    def model_fn(self, data, mode=None):
        ...
        with tf.variable_scope('lstm', reuse=tf.AUTO_REUSE):
            rnn_cell   = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(self.FLAGS.hidden_size) for _ in range(self.FLAGS.lstm_stack)])
            init_state = rnn_cell.zero_state(batch_size=self.FLAGS.batch_size, dtype=tf.float32)
        
            output, _ = tf.nn.dynamic_rnn(rnn_cell, sketch,
                                            initial_state=init_state,
                                            dtype=tf.float32,
                                            scope='lstm_dynamic'
                                  )
                
            dstates.update({'output': output})
        ...
        # lstm output logits --> logits for probabilities
        ...
        if mode in ['train', 'evaluate']:
            with tf.variable_scope('lstm/classification_loss', reuse=tf.AUTO_REUSE):
        ...
        if mode == 'train':
            ...
            with tf.variable_scope('lstm/loss', reuse=tf.AUTO_REUSE):
