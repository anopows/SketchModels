...
def kld_normal_isotropic(mu1, sigma1, mu2, sigma2, reduce_sum=False):
    ...
    with tf.name_scope("kld_normal_isotropic"):
        result = tf.reduce_sum(0.5 * (
            2 * tf.log(tf.maximum(1e-6, sigma2))
            - 2 * tf.log(tf.maximum(1e-6, sigma1))
            + (tf.square(sigma1) + tf.square(mu1 - mu2)) / tf.maximum(1e-6, (tf.square(sigma2))) - 1), keepdims=True, axis=-1)

        if reduce_sum is False:
            return result
        else:
            return tf.reduce_sum(result, reduce_sum) # Axis is a boolean? 
